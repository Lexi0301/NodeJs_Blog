...
import BowenxinxiModel from '../models/BowenxinxiModel'
...
export default ({ config, db }) => {
	...
	api.get('/page', async (req, res) => {
		...
		try {
			...
			var biaoqian = req.query.biaoqian
			if (biaoqian) {

				if (biaoqian.indexOf('%') != -1) {
					where.biaoqian = {
						[Op.like]: biaoqian
					}
				} else {
			...
			var bowenfenlei = req.query.bowenfenlei
			if (bowenfenlei) {

				if (bowenfenlei.indexOf('%') != -1) {
					where.bowenfenlei = {
						[Op.like]: bowenfenlei
					}
				} else {
			...
			let result = await BowenxinxiModel.findAndCountAll({
				order: [[sort, order]],
				where,
				offset: (page - 1) * limit,
				limit
	...
	api.post('/save', async (req, res) => {
		...
		try {
			...
			const userinfo = await BowenxinxiModel.create(req.body)
	...
	api.post('/update', async (req, res) => {
		...
		try {
			...
			await BowenxinxiModel.update(req.body, {
				where: {
				  id: req.body.id
				}
			})
	...
	api.all('/info/:id', async (req, res) => {
		...
		try {
			...
			const recordInfo = await BowenxinxiModel.findOne({ where: { id: req.params.id } })
            await recordInfo.increment('clicknum')

			toRes.record(res, 0, await BowenxinxiModel.findOne({ where: { id: req.params.id } }))
		} catch(err) {
	...
	api.get('/remind/:columnName/:type', async (req, res) => {
		...
		try {
			...
			if (req.params.type == 1) {
				if (req.query.remindstart) sql = "SELECT COUNT(*) AS count FROM bowenxinxi WHERE " + req.params.columnName + " >= '" + req.query.remindstart + "'"
				if (req.query.remindend) sql = "SELECT COUNT(*) AS count FROM bowenxinxi WHERE " + req.params.columnName + " <= '" + req.query.remindend + "'"

				if (req.query.remindstart && req.query.remindend) {
					sql = "SELECT COUNT(*) AS count FROM bowenxinxi WHERE " + req.params.columnName + " >= '" + req.query.remindstart + "' AND " + req.params.columnName + " <= '" + req.query.remindend + "'"
				}
			}
			...
			if (req.params.type == 2) {
				if (req.query.remindstart) {
					...
					sql = "SELECT COUNT(*) AS count FROM bowenxinxi WHERE " + req.params.columnName + " >= '" + remindStart + "'"
				}
				if (req.query.remindend) {
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM bowenxinxi WHERE " + req.params.columnName + " <= '" + remindEnd + "'"
				}

				if (req.query.remindstart && req.query.remindend) {
					let remindStart = util.getDateTimeFormat(0 - req.query.remindstart, "yyyy-MM-dd")
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM bowenxinxi WHERE " + req.params.columnName + " >= '" + remindStart + "' AND " + req.params.columnName + " <= '" + remindEnd + "'"
				}
			}
